...
export default function TableOrdersPage() {
  ...
  const filtered = orders.filter((o) => o.mode === "table" && (filter === "all" || o.status === filter));
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {FILTERS.map(({ label, value }) => (
          <button key={value} onClick={() => setFilter(value)}
            className={cn("px-4 py-2 rounded-full font-sans text-sm font-semibold whitespace-nowrap transition-colors shrink-0",
        ...
        {filtered.map((order) => (
          <div key={order.id} className="bg-white rounded-2xl border border-brand-warm-gray shadow-sm overflow-hidden">
            <button onClick={() => setExpanded(expanded === order.id ? null : order.id)} className="w-full flex items-center gap-3 p-4 text-left">
              ...
              {order.tableNumber && (
                <span className="bg-brand-brown text-white font-bold font-sans text-sm px-3 py-1 rounded-full shrink-0">
                  {order.tableNumber}
                </span>
              )}
              ...
              <span className={cn("text-xs font-sans font-semibold shrink-0 px-2 py-0.5 rounded-full",
                order.type === "parcel" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700")}>
                {order.type === "parcel" ? "Parcel" : "Table"}
              </span>
            ...
            <AnimatePresence>
              {expanded === order.id && (
                <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden border-t border-brand-warm-gray">
                  <div className="p-4 space-y-3">
                    {order.customerPhone && (
                      <p className="font-sans text-sm"><span className="text-brand-brown-mid">Phone:</span> <span className="text-brand-brown font-medium">{order.customerPhone}</span></p>
                    )}
                    <div className="space-y-2">
